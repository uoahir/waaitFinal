     ...
     <div class="card mt-3" id="kanbanBoard" style="height: 800px;display: flex; flex-direction: row;">
 		<div id="todo" style="width: 25%; border: 1px solid black; background-color: #d3e5ff">
 			...
 			<c:if test="${not empty teamProject.allocationList}">
 				<c:forEach var="allocation" items="${teamProject.allocationList}">
 					<c:if test="${allocation.functionStartDate eq null }">
 						  <p class ="mr-5 badge bg-primary" onclick="allocationtodo(event,${allocation.projectNo},${allocation.empNo});">${allocation.allocationFun}</p>      
 					</c:if>
 				</c:forEach>
 			</c:if>
 			</div>
 			...
 			<c:if test="${not empty teamProject.allocationList}">
 				<c:forEach var="allocation" items="${teamProject.allocationList}">
 					<c:if test="${allocation.functionStartDate != null && allocation.functionEndDate == null}">
  					<p class="mr-5 badge bg-warning" onclick="allocationinProgress(event,${allocation.projectNo},${allocation.empNo});">${allocation.allocationFun}</p>      
					</c:if>
 				</c:forEach>
 			</c:if>
 			</div>
 		...
 		<div class="" style="flex-direction: row;">
 			<c:if test="${not empty teamProject.allocationList}">
 				<c:forEach var="allocation" items="${teamProject.allocationList}">
 					<c:if test="${allocation.functionStartDate != null && allocation.functionEndDate != null}">
  					<p class="mr-5 badge bg-primary" onclick="allocationtodo(event,${projectNo},${empNo});">${allocation.allocationFun}</p>      
					</c:if>
 				</c:forEach>
 			</c:if>
 			</div>
